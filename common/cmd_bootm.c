...
#define local_debug(fmt,args...) printf(fmt, ##args)
...
static int bootm_load_os(image_info_t os, ulong *load_end, int boot_progress)
{
	...
	puts ("OK\n");
	local_debug ("   kernel loaded at 0x%08lx, end = 0x%08lx\n", load, *load_end);
	if (boot_progress)
		show_boot_progress (7);

	if ((load < blob_end) && (*load_end > blob_start)) {
		local_debug ("images.os.start = 0x%lX, images.os.end = 0x%lx\n", blob_start, blob_end);
		local_debug ("images.os.load = 0x%lx, load_end = 0x%lx\n", load, *load_end);
                              ...
                              bootm_headers_t *images, ulong *os_data, ulong *os_len)
{
	...
	if (argc < 2) {
		img_addr = load_addr;
		local_debug ("*  kernel: default image load address = 0x%08lx\n",
		             load_addr);
#if defined(CONFIG_FIT)
	} else if (fit_parse_conf (argv[1], load_addr, &img_addr,
	                           &fit_uname_config)) {
		local_debug ("*  kernel: config '%s' from image at 0x%08lx\n",
		             fit_uname_config, img_addr);
	} else if (fit_parse_subimage (argv[1], load_addr, &img_addr,
	                               &fit_uname_kernel)) {
		local_debug ("*  kernel: subimage '%s' from image at 0x%08lx\n",
		             fit_uname_kernel, img_addr);
#endif
	} else {
		img_addr = simple_strtoul(argv[1], NULL, 16);
		local_debug ("*  kernel: cmdline image address = 0x%08lx\n", img_addr);
	}
	...
	*os_data = *os_len = 0;
	...
	local_debug ("   kernel data at 0x%08lx, len = 0x%08lx (%ld)\n",
